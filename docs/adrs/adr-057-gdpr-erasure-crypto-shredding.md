...
- **Extended by `ADR-096`/`ADR-097`, added here — found un-cross-
  ...
  `ADR-097` was later removed as an adopted feature, 2026-09-04, see its
  own final additive note; `ADR-096`'s blind index/bucketed range remains
  adopted) on top of this ADR's crypto-shredded fields, plus a new erasure
  side-effect step in `EntityErasureResolver` to also purge the search index
