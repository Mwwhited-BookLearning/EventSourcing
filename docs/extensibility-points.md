...
| `ISearchIndexKeyStore` | Where a `Shared`-scope searchable-index HMAC key (one per `AppId`/event type/field, distinct lifecycle from `IErasureKeyStore`'s per-entity DEKs — manual rotation only, never auto-destroyed) is held | Designed, not yet built (`08-build-plan.md`'s matching item is Not started) | Keyed by `(AppId, EventTypeName, FieldJsonPath)` | `ADR-096` |
