...
| `IErasureKeyStore` | Where per-entity crypto-shredding keys (DEKs) are wrapped/stored/destroyed | **Corrected, 2026-08-12, found by an independent design-compliance audit**: only `LocalErasureKeyStore` (dev, EF-table-backed) and `HashiCorpVaultErasureKeyStore` (self-hosted, on-prem) actually ship in `EventStore.Erasure` — verified directly. `ADR-057`'s own Decision names Azure Key Vault/AWS KMS/Google Cloud KMS too (cloud); those three are decided, not built — see `TODO.md`'s `ADR-057` entry, not restated here | Keyed by `AppId` — multiple backends can be active **simultaneously** in one deployment, unlike `IUpcastExpressionEvaluator` below | `ADR-057` |
...
| `ISearchIndexKeyStore` | Where a `Shared`-scope searchable-index HMAC key (one per `AppId`/event type/field, distinct lifecycle from `IErasureKeyStore`'s per-entity DEKs — manual rotation only, never auto-destroyed) is held | `LocalSearchIndexKeyStore` (EF-table-backed) plus `CloudSearchIndexKeyStoreAdapter` — one adapter wrapping any `IErasureKeyStore` backend (Azure Key Vault, AWS KMS, Google Cloud KMS, HashiCorp Vault), not four bespoke SDK integrations | Keyed by `(AppId, EventTypeName, FieldJsonPath)` — several backends may be active simultaneously in one deployment, same multi-backend shape as `IErasureKeyStore` | `ADR-096` |
| `IEncryptedPredicateEvaluator` | Where the exact-match decrypt-and-compare step over an already-narrowed candidate set runs — application tier vs. a provider-native in-database function | `AppTierEncryptedPredicateEvaluator` (default; per-provider native evaluators designed, not yet built — see `ADR-098`'s honest per-provider feasibility notes) | One per deployment, never a full-table scan | `ADR-098` |
