...
rest:** [Masking content strategies](masking-strategies.md) has decided,
built winners for three of its options (`PartialReveal` and keyed `Hash`,
both `ADR-009`/`ADR-052`, alongside the original `FixedValue`) but
explicitly no clear pick for the rest of its menu (tokenization,
generalization) — see its own row below and `docs/10-open-questions.md`.
...
| [Masking content strategies](masking-strategies.md) — configurable partial reveal vs. format-preserving masking vs. generalization/bucketing vs. tokenization | `PartialReveal` and keyed `Hash` decided (`ADR-009`/`ADR-052`); generalization/tokenization not yet | `PartialReveal` (format-preserving, e.g. an SSN rendered `XXX-XX-1234`) and `Hash` (a keyed HMAC via `Microsoft.Extensions.Compliance.Redaction`'s `HmacRedactor`, letting a caller correlate masked values without ever seeing the real one) both fit `ADR-009`'s claims-gated wrapper cleanly and are now built, shared with `ADR-052`'s streaming-channel redaction. Still undecided: generalization fits only as a single-value transform (never a k-anonymity guarantee); tokenization's separate-party reversal model doesn't fit the wrapper at all — it would need its own mechanism, not a `strategy` value |
