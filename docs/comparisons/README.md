...
| [Event response acknowledgment](event-response-acknowledgment.md) — leave "this event expects a tracked response within a window" to the application vs. a first-class registry `ExpectedResponse` capability | Deferred, directly in the comparison (no ADR — back-burnered, `docs/10-open-questions.md`) | Raised by naming IONM (same-session clinical ack) and pharmacovigilance's 15-day regulatory clock as concrete cases of the same shape; recommends waiting until both are built out as real watchers before generalizing, since their deadline semantics differ enough that one case might not predict the other's actual shape |
