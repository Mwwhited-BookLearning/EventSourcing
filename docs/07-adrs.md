...
| [097](adrs/adr-097-order-revealing-encryption-opt-in.md) | Order-Revealing Encryption (ORE) as an opt-in, loudly-gated real range-comparison mechanism — sibling to `ADR-096`, no-override guardrail on classified fields (built, benchmarked, then **removed as an adopted feature**, 2026-09-04 — see the ADR's own additive note; real implementation kept as a reference in `spikes/order-revealing-encryption/`) | Accepted |
...
| [108](adrs/adr-108-centralized-nuget-package-versioning.md) | Centralized NuGet package versioning via `Directory.Packages.props` (Central Package Management) — every package updated to latest stable, including two real major-version jumps; `client-web`'s npm side updated the same pass with the same standard | Accepted |
