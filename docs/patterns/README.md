...
| Order-Revealing Encryption (property-preserving range comparison) | A cryptographic construction whose ciphertexts a public compare function can order directly, without decryption — real range queries evaluated entirely by the database, at the cost of a published leakage-abuse attack against low-cardinality fields | `ADR-097` (opt-in, no-override guardrail on classified fields) | Catalog only |
