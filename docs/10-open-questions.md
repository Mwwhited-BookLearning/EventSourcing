...
| How does an external federated IdP's `sub` map to this framework's own `ActorId` for `Role`/`UserPermission` lookup (`ADR-047`)? A simple `sub == ActorId` convention would work for the common case but isn't mandated. | `ADR-047` | Not designed further here — flagged rather than assumed. |
| How does a new peer server (`ADR-033`) discover other sites when independently deployed, outside any single Aspire/`docker-compose` orchestration boundary? mDNS/DNS-SD were re-examined and are the wrong tool (LAN-scoped), not a fit for cross-internet, cross-site discovery. | `ADR-033`, re-raised during a `references.md` review | Not designed further here — a static seed-peer list or a discovery service are both plausible, neither chosen. |
| Should `AuthorityStatus` (`ADR-035`/`ADR-042`) and/or `ReaderTrustBasis` (`ADR-045`) be split into multiple independent axes — identity-assurance, authorization-validity, content/detection-confidence — instead of one collapsed status, per [NIST 800-63-3](https://pages.nist.gov/800-63-3/sp800-63-3.html)'s IAL/AAL/FAL model? See [the pattern doc](../patterns/multi-axis-authority-assurance.md) for the general shape. | Raised in conversation, generalizing `ADR-035`/`ADR-042`/`ADR-045` | Genuinely undecided: which axis (or combination) would gate `ADR-042`'s Entity Store fold, and whether this applies to the write side, the read side, or both, are all open. |
