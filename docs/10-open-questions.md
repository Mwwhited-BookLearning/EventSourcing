...
| 2 | Should `ADR-043`'s delegated, capped, time-boxed access grants get a revocation-before-expiry mechanism and an Event Log audit trail of issuance, or is "valid until its own `exp` claim says otherwise, with no issuance record" an accepted trade-off? | Surfaced while writing `docs/patterns/delegated-capped-time-boxed-access-grants.md` and `docs/patterns/self-attested-did-ucan-delegation.md` (a Phase 0 missing-documents pass) — confirmed as a real, pre-existing gap in `ADR-043`'s own text, not introduced by either doc | This is a real design fork, not a bug: self-verified offline delegation (the whole point of the DID/UCAN pattern) is structurally in tension with live revocation, which needs an online check. Deliberately left for a human decision per this session's standing rule — a found gap may have been intentionally accepted, not overlooked |
| 3 | Should `.github/workflows/ci.yml` and `.github/dependabot.yml` (`ADR-091`) be rebuilt, or does their 2026-08-27 deletion stand as the current, accepted state? | Phase 1 (full ADR review) — `ADR-091` and `08-build-plan.md` item 39 both still claimed these files existed and were Done; verified via `git log` that they were deliberately deleted per direct request, logged in `docs/changes/2026-08-28.md`, but neither doc was ever updated to say so | The deletion was deliberate, not accidental, but nothing on record says whether it was meant to be permanent or just a pause — the underlying commands it wired together are still verified working locally either way, so this is purely about whether CI orchestration itself is wanted back |
