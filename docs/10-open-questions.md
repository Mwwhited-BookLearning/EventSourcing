...
|---|---|---|---|
| 1 | Should user flows/validations/approvals be expressed via a general-purpose DSL (e.g. a BPMN/rules-engine-style language), or should this repo keep composing its existing bespoke, single-purpose mechanisms — `RequiredSignature`/step-up (`ADR-066`), gated `authorityDecision` publish (`ADR-042`/`ADR-023`/`ADR-035`), `ExpectedResponse` tracking (`ADR-094`) — further as new cases arise? | Direct user request ("searching for good DSLs to use to define user flows, validations, approvals and so on"), TODO.md | Genuinely unresearched: no BPMN/rules-engine/workflow-engine DSL evaluation exists anywhere in `docs/references.md`, `docs/comparisons/`, or any ADR. A real fork, not a decided task — needs a `docs/comparisons/` doc weighing a real DSL/engine candidate against "keep composing existing primitives," per this project's own buy-over-build and verify-before-citing rules, before either path is chosen. |
