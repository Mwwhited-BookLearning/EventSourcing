...
|---|---|---|---|
| 1 | Should a framework-level `EventTypeDefinition.ExpectedResponse { EventType, Within }` capability (plus a reserved `ExpectedResponseMissing` detector event, mirroring `ADR-031`'s `ChannelLagDetected`) exist for event types that expect a tracked response within a window — or does this stay an application-level convention? | IONM (near-real-time alert acknowledgment) and polysomnography named as concrete telemetry SLA profiles (`docs/domains/clinical-trials-device-telemetry/README.md`'s "Special concerns"), which surfaced the related, distinct question of *tracked acknowledgment* | **Back-burnered.** `docs/comparisons/event-response-acknowledgment.md` weighs both options and recommends deferral: IONM's same-session clinical ack and pharmacovigilance's calendar-day regulatory 15-day clock (`docs/domains/pharmacovigilance/README.md`) are the only two concrete cases so far, and their deadline semantics differ enough that generalizing from just one risks guessing at a shape the other wouldn't fit. Revisit once both are built out as real feature docs and the watcher logic is checked for actual convergence — not before. |
