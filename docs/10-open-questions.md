...
|---|---|---|---|
| 1 | Should this framework adopt a DSL for user flows/validations/approvals (a declarative way to express a multi-step business process, beyond what `ADR-101`'s PlantUML-native flow engine already covers for read-side pending-task derivation)? | `TODO.md`'s own historical note ("moved to `docs/10-open-questions.md` row 1, not kept here") — restored here, `2026-09-04`, after this row's own number was found repurposed for the (now-resolved, `ADR-107`) delegated-grant-issuance question with no forwarding note, leaving this one silently missing. | Genuinely undecided fork, not decided work with only the doing left — no direct request has settled whether a DSL is wanted, what shape it would take, or whether `ADR-101`'s existing PlantUML-native engine already covers enough of this need. |
