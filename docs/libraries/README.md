...
| [CEL for .NET (`Cel.NET`)](dotnet/cel-dotnet.md) | dotnet | Declarative expression evaluation for common upcast mappings — the default `IUpcastExpressionEvaluator` | The .NET port ecosystem is still fragmented (several small community packages, no single dominant one), but `Cel.NET` specifically was spiked and verified working before adoption (build-plan item 11) — see `docs/libraries/dotnet/cel-dotnet.md` | The default, declarative upcast-mapping engine | `ADR-018`, `ADR-037`, `ADR-053` |
...
| ~~Polly + Simmy~~ | dotnet | ~~In-process fault injection — durable outbox/inbox crash-recovery testing~~ | **Removed** — Polly's new Open Source Maintenance Fee; replaced with a hand-rolled `FaultInjector`, see [`dotnet/polly-simmy.md`](dotnet/polly-simmy.md) | ~~Simulated-crash verification of `ADR-033`'s outbox/inbox resumption~~ | `ADR-063` |
...
| [System.Security.Cryptography.Pkcs](dotnet/system-security-cryptography-pkcs.md) | dotnet | RFC 3161 Time-Stamp Protocol request/response, CMS SignedData (BCL, first-party) | Ships a public reader (request-build/response-verify) but no public writer -- issuing a token is real TSA software's job, not this library's; this repo's own test-only fake TSA hand-builds the response wrapper against the raw ASN.1 for exactly that reason | Client-side RFC 3161 timestamp request/response handling, no third-party crypto library | `ADR-086` |
