...
Domain-agnostic capabilities owned by `App.vue`/`@eventstore/mvvm-client`
itself, not by any Vitals/Meridian persona — `core` is this table's own
stand-in for "domain" in the `{domain}/{role}/{task}.md` naming
convention above, since no proving-ground persona owns these screens.

| Playbook | Role | Feature doc | Generated by |
|---|---|---|---|
| [Go Offline and Resync](core/user/go-offline-and-resync.md) | User | [`pwa-offline-outbox.md`](../patterns/pwa-offline-outbox.md) | `OfflineOutboxSyncPlaybookTests.RecordOfflineOutboxSyncPlaybook` |
