...
// the strategies). ADR-057 made PayloadMasker also depend on ErasureKeyService
// (a real, DI-resolved dependency, not optional the way PublishService's own
// PayloadEncryptor is) -- db/registry are the caller's own already-constructed
// instances, reused here rather than building a second, divergent set.
internal static class MaskingTestSupport
{
    ...
    public static (IPayloadMasker PayloadMasker, CapturingLoggerProvider Logs) CreatePayloadMasker(EventStoreContext db, SchemaRegistryService registry)
    {
        ...
        services.AddErasure(new ConfigurationBuilder().Build()); // no Erasure:Vault:Address configured -- "Local" backend only, same as every Host's dev default
