...
import { useEventComposer, type ComposerFormField, type EventTypeSummary } from '../../composables/useEventComposer'
...
async function selectEventType(summary: EventTypeSummary): Promise<void> {
  ...
  statusMessage.value = ''
  fields.value = await composer.getFormFields(summary.name, summary.version)
  formValues.value = {}
  for (const field of fields.value) if (field.type === 'boolean') formValues.value[field.name] = false
}

const canPublish = computed(() => selected.value !== null && fields.value.filter((f) => f.required && f.editable).every((f) => formValues.value[f.name]))
...
async function submit(): Promise<void> {
  ...
  statusMessage.value = 'Publishing...'
  const result = await composer.publish(selected.value.name, payload)
  statusMessage.value = result.ok
    ? `Published -- status: ${result.status}, entityId: ${result.entityId}`
